...
def step_seven(stream, app, figs, histograms, lines, histograms_alg, lines_alg, figs_alg,
    ...
    while continue_stream != last_frame:
        ...
        stream.roi_a = stream.current_frame_a[
                     int(y_a - n_sigma * stream.static_sigmas_y): y_a + int(n_sigma * stream.static_sigmas_y + 1),
                     int(x_a - n_sigma * stream.static_sigmas_x): x_a + int(n_sigma * stream.static_sigmas_x + 1)
                     ]
