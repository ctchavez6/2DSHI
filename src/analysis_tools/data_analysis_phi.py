...
def read_in_csv(path_to_phi_csv):
    phi_sample_csv_file = pandas.read_csv(path_to_phi_csv, header=None)
    return phi_sample_csv_file.values

def generate_images_from_PHI_MATRIX__(PHI_MATRIX__, csv_filename, shape_params, user_shape_choice):
    data_dictionary = dict()

    if user_shape_choice == 1:  # Circle
        ...
        with warnings.catch_warnings():
            ...
            values_phi_sample = PHI_MATRIX__
            ...
            DISPLAYABLE_PHI_MATRIX__ = np.zeros((SIN_PHI_MATRIX.shape[0], SIN_PHI_MATRIX.shape[1], 3), dtype=np.uint8)
            DISPLAYABLE_PHI_MATRIX__[:, :, 1] = np.where(SIN_PHI_MATRIX < 0.00, abs(SIN_PHI_MATRIX * (2 ** 8 - 1)), 0)
            DISPLAYABLE_PHI_MATRIX__[:, :, 0] = np.where(SIN_PHI_MATRIX < 0.00, abs(SIN_PHI_MATRIX * (2 ** 8 - 1)), 0)

            DISPLAYABLE_PHI_MATRIX__[:, :, 0] = np.where(SIN_PHI_MATRIX > 0.00, abs(SIN_PHI_MATRIX * (2 ** 8 - 1)),
                                                       DISPLAYABLE_PHI_MATRIX__[:, :, 0])


        # This is the raw csv -> png with no circles/spirals/lines added
        image = Image.fromarray(DISPLAYABLE_PHI_MATRIX__.astype('uint8'), 'RGB')
        image.save(csv_filename.replace(".csv", ".png"))


        spiral = np.asarray(DISPLAYABLE_PHI_MATRIX__[:, :, :])
        ...
        for radius in shape_params["radii"]:
            ...
            for (cord_y, cord_x) in zip(spiral_coords_y, spiral_coords_x):
                ...
                r_values.append(PHI_MATRIX__[cord_y, cord_x])
                data_dictionary["r={}".format(radius)].append(PHI_MATRIX__[cord_y, cord_x])
    ...
    if user_shape_choice == 3:  # Horiztal or Vertical Lines
        with warnings.catch_warnings():
            warnings.filterwarnings('ignore', r'All-NaN (slice|axis) encountered')
            ...
            SIN_PHI_MATRIX = np.sin(values_phi_sample)
        ...
        # Raw csv -> png: have not added lines yet
        image = Image.fromarray(DISPLAYABLE_PHI_MATRIX__.astype('uint8'), 'RGB')
        image.save(csv_filename.replace(".csv", ".png"))

        # sin_phi_with_lineouts_img is the output png with the lineouts
        sin_phi_with_lineouts_img = np.asarray(DISPLAYABLE_PHI_MATRIX__[:, :, :])
        height = sin_phi_with_lineouts_img.shape[0]
        ...
        scale_factor = 5.86*(10**(-3))*4.09
        data_dictionary["x_values"] = [i*scale_factor for i in range(len(PHI_MATRIX__[0, :]))]
        for y_location in line_y_locations:
            sin_phi_with_lineouts_img[y_location, :, :] = 255

            data_dictionary["y={}".format(y_location)] = np.sin(PHI_MATRIX__[y_location, :])  # For the csv lineout data

            sin_phi_values = np.sin(PHI_MATRIX__[y_location, :]) # For the plot outputs
            data_point_indices = [i*scale_factor for i in range(len(sin_phi_values))]

            fig = plt.figure()
            plt.plot(data_point_indices, sin_phi_values)
            plt.title("{}, Sin(Phi)_Values_Over_Horizontal_Lineout\ny = {}".format(frame_title, y_location))
            plt.savefig(os.path.join(get_data_directory(csv_filename), "Sin(Phi)_Values_Over_y={}.png".format(y_location)))
            plot_paths.append(path.join(get_data_directory(csv_filename), "Sin(Phi)_Values_Over_y={}.png".format(y_location)))
        ...
        pil_img = Image.fromarray(sin_phi_with_lineouts_img.astype('uint8'), 'RGB')
...
def vertically_stack_all_these_images(parent_folder, paths_to_images):
    ...
    imgs_comb.save(os.path.join(parent_folder, 'Sin(Phi)_Values_Over_Horizontal_Lineout.png'))
...
phi_array = read_in_csv(phi_csv)
if user_choice == 1:
    print("Shape: Circle(s)")
    generate_images_from_PHI_MATRIX__(phi_array, phi_csv, circle_options, user_choice)
    vertically_stack_all_these_images(run_dir, plot_paths)
    delete_all_sub_images(plot_paths)
elif user_choice == 3:
    print("Shape: Lines(s)")
    generate_images_from_PHI_MATRIX__(phi_array, phi_csv, h_lines_options, user_choice)
    vertically_stack_all_these_images(run_dir, plot_paths)
    delete_all_sub_images(plot_paths)
else:
